...
export interface AutoOption {
  ...
  label?: string;             // display; defaults to value
  meta?: any;                 // extra data returned on pick
  crest_url?: string;         // optional visual
  color?: string;             // optional visual
  aliases?: string[];         // extra matchable text
}

interface Props {
  value: string;
  onChange: (v: string, picked?: AutoOption) => void;
  options: AutoOption[];
  placeholder?: string;
  className?: string;
  maxSuggestions?: number;
  onPick?: (opt: AutoOption) => void;
}

// Fuzzy search: matches if every char of query appears in order in the target,
// scored by contiguous matches. Cheap and pleasant.
...
export default function Autocomplete({ value, onChange, options, placeholder, className, maxSuggestions = 8, onPick }: Props) {
  ...
  const suggestions = q
    ? options
        .map((o) => {
          const targets = [o.value, o.label ?? '', ...(o.aliases ?? [])].filter(Boolean);
          const s = Math.max(...targets.map((t) => score(t, q)));
          return { o, s };
        })
        .filter((r) => r.s >= 0)
        .sort((a, b) => b.s - a.s)
  ...
  function pick(o: AutoOption) {
    onChange(o.label ?? o.value, o);
    onPick?.(o);
    setOpen(false);
  }

...
export default function TransfersSection({ seasonId }: { seasonId: string }) {
  ...
  async function load() {
    setLoading(true);
    const { data } = await supabase.from('transfers').select('*').eq('season_id', seasonId).order('created_at', { ascending: false });
    setRows(data ?? []);
    setLoading(false);
  }
  ...
  async function add() {
    ...
    const { error } = await supabase.from('transfers').insert({ ...form, season_id: seasonId });
    if (error) { alert(error.message); return; }
    setForm({ direction: 'in' }); setShowForm(false);
    load();
  }
  ...
  return (
    <div>
      ...
      {showForm && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-3 mb-4 grid gap-2 sm:grid-cols-3">
          <select value={form.direction ?? 'in'} onChange={(e) => setForm({ ...form, direction: e.target.value as any })} className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm">
            <option value="in">IN (bought/loan-in)</option>
            <option value="out">OUT (sold/loan-out)</option>
          </select>
          ...
          <input placeholder="Amount (150MM)" value={form.amount ?? ''} onChange={(e) => setForm({ ...form, amount: e.target.value })} className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm" />
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <TransferList title="IN" color="emerald" rows={ins} onDelete={del} />
        <TransferList title="OUT" color="red" rows={outs} onDelete={del} />
      </div>
    </div>
  );
}

function TransferList({ title, color, rows, onDelete }: { title: string; color: 'emerald' | 'red'; rows: Transfer[]; onDelete: (id: string) => void }) {
  return (
    <div>
      <div className={`text-xs uppercase font-semibold mb-1 text-${color}-600 dark:text-${color}-400`}>{title} ({rows.length})</div>
      <div className="border border-slate-200 dark:border-slate-800 rounded overflow-hidden bg-white dark:bg-slate-900">
        {rows.length === 0 ? (
          <div className="text-center text-slate-400 dark:text-slate-600 py-6 text-sm">—</div>
        ) : rows.map((r) => (
          <div key={r.id} className="p-3 border-t first:border-t-0 border-slate-200 dark:border-slate-800 flex items-center gap-2">
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-slate-500 truncate">
                {r.from_team ? `${r.from_team} → ` : ''}{r.to_team ?? ''} {r.amount ? `· ${r.amount}` : ''}
              </div>
              {r.note && <div className="text-xs text-slate-400 mt-1">{r.note}</div>}
            </div>
            <button onClick={() => onDelete(r.id)} className="text-xs text-red-500 hover:text-red-600">×</button>
          </div>
        ))}
      </div>
    </div>

...
interface Contract {
  ...
  team_catalog_id: string | null;
  signed_year: number | null;
  signed_month: number | null;
  monthly_salary: number | null;
  monthly_salary_currency: string | null;
  notes: string | null;
  created_at: string;
}
...
export default function ContractsPage() {
  ...
  async function add() {
    ...
    const payload = {
      ...
      team_text_color: textColor || null,
      team_crest_url: crest || null,
      team_catalog_id: catalogId,
      signed_year: signedYear ? Number(signedYear) : null,
      signed_month: signedMonth ? Number(signedMonth) : null,
      monthly_salary: salary ? Number(salary) : null,
    };
    const { error } = await supabase.from('contracts').insert(payload);
    if (error) { alert('Error: ' + error.message); console.error(error); return; }
    setTeam(''); setColor('#059669'); setTextColor('#ffffff'); setCrest(''); setCatalogId(null);
    setSignedMonth(''); setSignedYear(''); setSalary('');
    setShowForm(false);
    load();
  }
  ...
  const teamOptions: AutoOption[] = teams.map((tt) => ({
    ...
    meta: { country: tt.country, primary_color: tt.primary_color, text_color: tt.text_color, crest_url: tt.crest_url, id: tt.id },
  }));
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-4 mb-6 grid gap-2 sm:grid-cols-2">
          <Autocomplete
            ...
            onChange={(v, opt) => {
              ...
              if (opt) {
                ...
                if (opt.meta?.text_color) setTextColor(opt.meta.text_color);
                if (opt.meta?.crest_url) setCrest(opt.meta.crest_url);
                if (opt.meta?.id) setCatalogId(opt.meta.id);
              }
            }}
            options={teamOptions}
            placeholder="Team (type to search)"
          />
          <div className="flex gap-2 items-center">
            ...
            <input value={color} onChange={(e) => setColor(e.target.value)} placeholder="#hex" className="flex-1 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm text-slate-900 dark:text-slate-100" />
          </div>
          <select value={signedMonth} onChange={(e) => setSignedMonth(e.target.value)} className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm text-slate-900 dark:text-slate-100">
            <option value="">Signed month…</option>
            {MONTHS.map((m, i) => <option key={m} value={i + 1}>{m}</option>)}
          </select>
          <input value={signedYear} onChange={(e) => setSignedYear(e.target.value)} placeholder="Signed year (2037)" className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm text-slate-900 dark:text-slate-100" />
          <input value={salary} onChange={(e) => setSalary(e.target.value)} placeholder="Monthly salary (EUR)" type="number" className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm text-slate-900 dark:text-slate-100" />
          <input value={crest} onChange={(e) => setCrest(e.target.value)} placeholder="Crest URL (auto if picked)" className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm text-slate-900 dark:text-slate-100" />
          <div className="sm:col-span-2 flex justify-end gap-2">
            <button onClick={() => setShowForm(false)} className="px-3 py-2 text-sm text-slate-500">{t('cancel')}</button>
            <button onClick={add} className="bg-emerald-600 hover:bg-emerald-500 text-white rounded px-4 py-2 text-sm font-medium">{t('save_btn')}</button>
          </div>
        </div>
      )}
      ...
      {rows.length === 0 ? (
        <div className="text-slate-500 dark:text-slate-400 border border-dashed border-slate-300 dark:border-slate-700 rounded p-8 text-center bg-white dark:bg-slate-900">
          No contracts yet. Add one to link a team + color + salary to your seasons.
        </div>
      ) : (
        <div className="grid gap-3">
          {rows.map((c) => (
            <div key={c.id} className="flex items-center gap-3 border border-slate-200 dark:border-slate-800 rounded-lg p-3 bg-white dark:bg-slate-900">
              ...
              <button onClick={() => del(c.id)} className="text-xs text-red-500 hover:text-red-600">×</button>
            </div>
          ))}
        </div>
      )}

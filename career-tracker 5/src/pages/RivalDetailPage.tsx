...
export default function RivalDetailPage() {
  ...
  async function addMatch() {
    ...
    const hg = form.home_goals ?? 0;
    const ag = form.away_goals ?? 0;
    const homeIsMine = (form.home_team ?? '').toLowerCase() !== (rival?.rival_team ?? '').toLowerCase();
    const myGoals = homeIsMine ? hg : ag;
    const theirGoals = homeIsMine ? ag : hg;
    ...
    <div>
      ...
      <div className="mt-3 rounded-lg p-5 mb-4 flex items-center gap-4" style={{ background: rival.rival_color ?? '#0f172a', color: '#fff' }}>
        ...
        <div>
          ...
          <div className="opacity-80 text-sm mt-1">{matches.length} {t('matches')}</div>
        </div>
      </div>

      <div className="grid grid-cols-5 gap-2 mb-6">
        <Stat label={t('wins')} value={w} color="emerald" />
        <Stat label={t('draws')} value={d} color="slate" />
        <Stat label={t('losses')} value={l} color="red" />
        <Stat label={t('goals_for')} value={gf} color="emerald" />
        <Stat label={t('goals_against')} value={ga} color="red" />
      </div>

      <div className="flex justify-end mb-3">
        <button onClick={() => setShowForm((v) => !v)} className="bg-emerald-600 hover:bg-emerald-500 text-white rounded px-4 py-2 text-sm">+ {t('new_match')}</button>
      </div>
      ...
      {showForm && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-4 mb-6 grid gap-2 sm:grid-cols-3">
          ...
          <input value={form.scorers ?? ''} onChange={(e) => setForm({ ...form, scorers: e.target.value })} placeholder="Scorers (comma sep)" className="sm:col-span-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm" />
          <div className="sm:col-span-3 flex justify-end gap-2">
            <button onClick={() => setShowForm(false)} className="px-3 py-2 text-sm text-slate-500">{t('cancel')}</button>
            <button onClick={addMatch} className="bg-emerald-600 hover:bg-emerald-500 text-white rounded px-4 py-2 text-sm">{t('save_btn')}</button>
          </div>
        </div>
      )}
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-xs uppercase">
            <tr>
              ...
              <th className="text-center px-3 py-2 w-12">R</th>
            </tr>
          </thead>
          <tbody>
            {matches.length === 0 ? (
              <tr><td colSpan={6} className="text-center text-slate-400 dark:text-slate-600 py-6">No matches logged yet.</td></tr>
            ) : matches.map((m) => (
              <tr key={m.id} className="border-t border-slate-200 dark:border-slate-800">
                ...
                <td className="px-3 py-2 text-center">
                  <span className={`inline-block w-6 h-6 rounded text-xs leading-6 font-bold text-white ${
                    m.result === 'W' ? 'bg-emerald-600' : m.result === 'L' ? 'bg-red-600' : 'bg-slate-400'
                  }`}>{m.result ?? '?'}</span>
                </td>
              </tr>
            ))}

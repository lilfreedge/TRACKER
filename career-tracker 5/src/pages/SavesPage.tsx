...
export default function SavesPage() {
  ...
  async function load() {
    ...
    const { data, error } = await supabase
      ...
      .eq('game_edition', decodedEdition)
      .order('sort_order', { ascending: true })
      .order('created_at', { ascending: false });
    if (error) { console.error('[saves load]', error); alert('Load error: ' + error.message); }
  ...
  async function createSave() {
    ...
    try {
      const { data, error } = await supabase.from('career_saves').insert({
        ...
        game_edition: decodedEdition,
        is_active: true,
      }).select();
      if (error) {
        console.error('[createSave] error', error);
        alert('Error adding save: ' + error.message + (error.hint ? '\n' + error.hint : ''));
        return;
      }
      console.log('[createSave] inserted', data);
      setNewName('');
      await load();
    } catch (e: any) {
      console.error('[createSave] threw', e);

...
export default function InjuriesSection({ seasonId }: { seasonId: string }) {
  ...
  async function del(id: string) { if (!confirm('Delete?')) return; await supabase.from('injuries').delete().eq('id', id); load(); }
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-3 mb-4 grid gap-2 sm:grid-cols-3">
          ...
          <input placeholder="Injury (ACL)" value={form.injury_type ?? ''} onChange={(e) => setForm({ ...form, injury_type: e.target.value })} className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm" />
      ...
      <div className="border border-slate-200 dark:border-slate-800 rounded overflow-hidden bg-white dark:bg-slate-900">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-xs uppercase">
            <tr><th className="text-left px-3 py-2">Player</th><th className="text-left px-3 py-2">Injury</th><th className="text-left px-3 py-2">Duration</th><th className="text-left px-3 py-2">Replaced by</th><th className="text-left px-3 py-2">Date</th><th></th></tr>
          </thead>
          <tbody>
            {rows.length === 0 ? (<tr><td colSpan={6} className="text-center text-slate-400 dark:text-slate-600 py-6">No injuries logged.</td></tr>) : rows.map((r) => (

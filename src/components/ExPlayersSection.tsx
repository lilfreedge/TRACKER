...
export default function ExPlayersSection({ seasonId }: { seasonId: string }) {
  ...
  async function load() {
    setLoading(true);
    const { data } = await supabase.from('ex_players').select('*').eq('season_id', seasonId).order('year_gone', { ascending: false });
    setRows(data ?? []);
    setLoading(false);
  }
  ...
  async function add() {
    ...
    const payload: any = { ...form, season_id: seasonId };
    if (payload.ovr === '') payload.ovr = null;
    if (payload.seasons_played === '') payload.seasons_played = null;
    if (payload.year_gone === '') payload.year_gone = null;
    const { error } = await supabase.from('ex_players').insert(payload);
    if (error) { alert(error.message); return; }
    setForm({}); setShowForm(false);
    load();
  }
  async function del(id: string) {
    if (!confirm('Delete this ex-player?')) return;
    await supabase.from('ex_players').delete().eq('id', id);
    load();
  }
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-3 mb-4 grid gap-2 sm:grid-cols-3">
          ...
          <input placeholder="Value (150MM)" value={form.value ?? ''} onChange={(e) => setForm({ ...form, value: e.target.value })} className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm" />
          ...
          <input placeholder="Seasons played" type="number" value={form.seasons_played ?? ''} onChange={(e) => setForm({ ...form, seasons_played: e.target.value ? Number(e.target.value) : null })} className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm" />
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-xs uppercase">
            <tr>
              ...
              <th className="text-left px-3 py-2">Now at</th>
              <th className="text-right px-3 py-2">Value</th>
              <th className="text-right px-3 py-2">Gone</th>
              <th></th>
            </tr>
          </thead>
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={7} className="text-center text-slate-400 dark:text-slate-600 py-6">No ex-players.</td></tr>
            ) : rows.map((r) => (
              <tr key={r.id} className="border-t border-slate-200 dark:border-slate-800">
                ...
                <td className="px-3 py-2 text-slate-600 dark:text-slate-300">{r.current_team ?? '—'}</td>
                <td className="px-3 py-2 text-right">{r.value ?? '—'}</td>
                <td className="px-3 py-2 text-right text-slate-500">{r.year_gone ?? '—'}</td>
                <td className="px-3 py-2 text-right"><button onClick={() => del(r.id)} className="text-xs text-red-500 hover:text-red-600">×</button></td>
              </tr>
            ))}
          </tbody>
        </table>

...
export default function CareerPage() {
  ...
  for (const r of results) if (r.result?.toLowerCase() === 'winner') trophyMap.set(r.competition_name_snapshot, (trophyMap.get(r.competition_name_snapshot) ?? 0) + 1);
  const trophies = [...trophyMap.entries()].sort((a, b) => b[1] - a[1]);
  const total = [...trophyMap.values()].reduce((a, b) => a + b, 0);
  return (
    <div>
      ...
      {trophies.length > 0 && (
        <div className="mb-6">
          <div className="text-xs uppercase text-slate-500 mb-2">Trophies by competition</div>
          <div className="border border-slate-200 dark:border-slate-800 rounded overflow-hidden bg-white dark:bg-slate-900">
            <table className="w-full text-sm">
              <tbody>{trophies.map(([c, n]) => (<tr key={c} className="border-t first:border-t-0 border-slate-200 dark:border-slate-800"><td className="px-3 py-2">{c}</td><td className="px-3 py-2 text-right font-bold text-emerald-600">×{n}</td></tr>))}</tbody>
            </table>
          </div>
        </div>
      )}
      <div>
        ...
        <div className="grid gap-2">
          {seasons.map((s) => (
            <Link key={s.id} to={`/season/${s.id}`} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded p-3 flex items-center gap-3 hover:border-emerald-400">
              {s.team_color && (<div className="w-2 h-8 rounded" style={{ background: s.team_color }} />)}
              <div className="flex-1"><div className="font-medium">{s.label}</div><div className="text-xs text-slate-500">{s.team_name_snapshot ?? '—'}</div></div>
            </Link>
          ))}
        </div>
      </div>

...
export default function DashboardPage() {
  ...
  async function load() {
    ...
    if (ss && ss.length) {
      ...
      setResults((rs ?? []) as SeasonResult[]); setPlayers((ps ?? []) as SquadPlayer[]);
    }
    setLoading(false);
  }
  ...
  const trophies = results.filter((r) => (r.result ?? '').toLowerCase().startsWith('winn')).length;
  ...
  const topByOvr = [...players].filter((p) => p.ovr != null).sort((a, b) => (b.ovr ?? 0) - (a.ovr ?? 0)).slice(0, 8);
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[['Seasons', seasons.length, ''], ['Clubs managed', teams.size, ''], ['Trophies', trophies, 'text-emerald-600'], ['Runners-up', runners, 'text-amber-600']].map(([l, v, c]) => (
      ...
      <div className="grid gap-4 md:grid-cols-2 mb-6">
        <div className="border border-slate-200 dark:border-slate-800 rounded-lg p-4 bg-white dark:bg-slate-900">
          <div className="text-xs uppercase text-slate-500 mb-2">Squad</div>
        ...
        <div className="border border-slate-200 dark:border-slate-800 rounded-lg p-4 bg-white dark:bg-slate-900">
          <div className="text-xs uppercase text-slate-500 mb-2">Career length</div>
          <div className="text-sm">{seasons.length} seasons across {teams.size} club{teams.size === 1 ? '' : 's'}.</div>
        </div>
      </div>
      <div>
        <div className="text-xs uppercase text-slate-500 mb-2">Top 8 by OVR</div>
        <div className="border border-slate-200 dark:border-slate-800 rounded overflow-hidden bg-white dark:bg-slate-900">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-800 text-slate-500 text-xs uppercase"><tr><th className="text-left px-3 py-2">Name</th><th className="text-left px-3 py-2">Pos</th><th className="text-right px-3 py-2">OVR</th></tr></thead>
            <tbody>{topByOvr.length === 0 ? (<tr><td colSpan={3} className="text-center text-slate-400 py-4">—</td></tr>) : topByOvr.map((p) => (<tr key={p.id} className="border-t border-slate-200 dark:border-slate-800"><td className="px-3 py-2"><Link to={`/player/${p.id}`} className="hover:underline">{p.name_snapshot}</Link></td><td className="px-3 py-2 font-mono">{p.position ?? '?'}</td><td className="px-3 py-2 text-right font-semibold">{p.ovr}</td></tr>))}</tbody>
          </table>
        </div>
      </div>
    </div>

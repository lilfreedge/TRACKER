...
export default function SavePage() {
  ...
  function moveTile(idx: number, dir: -1 | 1) {
    setTileOrder((prev) => { const next = [...prev]; const to = idx + dir; if (to < 0 || to >= next.length) return prev; [next[idx], next[to]] = [next[to], next[idx]]; try { localStorage.setItem(`tileOrder:${saveId}`, JSON.stringify(next)); } catch {} return next; });
  }
  async function createSeason() {
    ...
    setLabel(''); setNationalTeam(''); setNationalSinceMonth(''); setNationalSinceYear(''); setShowForm(false); load();
  }
  async function moveSeason(idx: number, dir: -1 | 1) {
    const next = [...seasons]; const to = idx + dir;
    if (to < 0 || to >= next.length) return;
    [next[idx], next[to]] = [next[to], next[idx]]; setSeasons(next);
    for (let i = 0; i < next.length; i++) await supabase.from('seasons').update({ sort_order: i }).eq('id', next[i].id);
  }
    ...
    <div>
      ...
      <div className="mt-2 mb-6 flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="flex gap-2">
          <button onClick={() => navigate(`/save/${save.id}/contracts`)} className="bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded px-4 py-2 text-sm font-medium flex items-center gap-1.5"><ContractIcon width="16" height="16" /> New contract</button>
          <button onClick={() => setShowForm((v) => !v)} className="bg-emerald-600 hover:bg-emerald-500 text-white rounded px-4 py-2 text-sm font-medium flex items-center gap-1"><PlusIcon width="16" height="16" /> {t('new_season')}</button>
        </div>
      </div>
      <div className="flex items-center justify-between mb-2">
        <div className="text-xs uppercase text-slate-500 dark:text-slate-400">Overview</div>
        <button onClick={() => setReorderMode((v) => !v)} className="text-xs text-slate-500 hover:text-slate-700 dark:hover:text-slate-300">{reorderMode ? 'Done' : 'Reorder'}</button>
      </div>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        {tileOrder.map((k, i) => {
          const tile = tiles[k]; const Icon = tile.icon;
          return (
            <div key={k} className={`relative border rounded-lg p-4 transition ${tile.accent} ${tile.disabled ? 'opacity-40 cursor-not-allowed' : 'hover:shadow-sm cursor-pointer'}`} onClick={() => !tile.disabled && !reorderMode && tile.onClick()}>
      ...
      {showForm && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-4 mb-6 grid gap-2 sm:grid-cols-3">
          <input value={nationalTeam} onChange={(e) => setNationalTeam(e.target.value)} placeholder={t('national_team_placeholder')} className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm" />
          <select value={nationalSinceMonth} onChange={(e) => setNationalSinceMonth(e.target.value)} className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm">
            <option value="">Since month…</option>
            {['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'].map((m, i) => <option key={m} value={i + 1}>{m}</option>)}
          </select>
          <input value={nationalSinceYear} onChange={(e) => setNationalSinceYear(e.target.value)} placeholder="Since year (2037)" className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm" />
          <input value={label} onChange={(e) => setLabel(e.target.value)} placeholder={t('label_placeholder')} className="sm:col-span-3 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm" />
          <div className="sm:col-span-3 flex justify-end gap-2">
            <button onClick={() => setShowForm(false)} className="px-3 py-2 text-sm text-slate-500">{t('cancel')}</button>
      ...
      {seasons.length === 0 ? (
        <div className="text-slate-500 dark:text-slate-400 border border-dashed border-slate-300 dark:border-slate-700 rounded p-8 text-center bg-white dark:bg-slate-900">{t('empty_seasons')}</div>
      ) : (
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {seasons.map((s, idx) => (
            <div key={s.id} className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-3 hover:border-emerald-400 hover:shadow-sm transition flex items-center gap-3 group">
              {s.team_color && (<div className="w-1 h-8 rounded-sm" style={{ background: s.team_color }} />)}
              <Link to={`/season/${s.id}`} className="flex-1 min-w-0">
                <div className="font-medium text-slate-900 dark:text-slate-100 truncate">{s.team_name_snapshot ?? '—'}</div>
                <div className="text-slate-500 dark:text-slate-400 text-xs mt-0.5 truncate">{s.label}{s.club_since_year ? ` · ${t('since')} ${s.club_since_year}` : ''}{s.national_team ? ` · 🌍 ${s.national_team}` : ''}</div>
              </Link>
              ...
              <button onClick={(e) => { e.preventDefault(); setOpenInfoId(openInfoId === s.id ? null : s.id); }} className="text-slate-400 hover:text-slate-600" title={t('registered')}><InfoIcon width="16" height="16" /></button>
              {reorderMode && (
              ...
              {openInfoId === s.id && (<div className="absolute right-2 top-full mt-1 bg-slate-900 text-white text-xs rounded px-3 py-2 shadow-lg z-10 whitespace-nowrap">{t('registered')}: {formatDate(s.created_at)}</div>)}
            </div>
          ))}
        </div>
      )}
...
function formatDate(iso: string): string { const d = new Date(iso); const m = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']; return `${m[d.getMonth()]} ${d.getFullYear()}`; }

...
interface RivalStats extends Rival { wins: number; draws: number; losses: number; goals_for: number; goals_against: number; }
interface TeamRow { id: string; name: string; country: string | null; primary_color: string | null; crest_url: string | null; aliases: string[] | null; }
export default function RivalsPage() {
  ...
  async function load() {
    ...
    setRivals(stats); setLoading(false);
  }
  useEffect(() => { load(); }, [saveId]);
  async function addRival() {
    const name = team.trim();
    if (!saveId || !name) { alert('Enter a team'); return; }
    const { error } = await supabase.from('rivals').insert({ save_id: saveId, rival_team: name, rival_color: color, notes: logoUrl ? `logo:${logoUrl}` : null });
    if (error) { alert('Error: ' + error.message); return; }
    setTeam(''); setLogoUrl(''); setColor('#DA291C'); load();
  }
  const teamOptions: AutoOption[] = teams.map((tt) => ({ value: tt.name, label: tt.name, aliases: tt.aliases ?? [], crest_url: tt.crest_url ?? undefined, color: tt.primary_color ?? undefined, meta: { country: tt.country, primary_color: tt.primary_color, crest_url: tt.crest_url } }));
  ...
  return (
    <div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-4 mb-6 grid gap-2 sm:grid-cols-4">
        ...
        <input value={logoUrl} onChange={(e) => setLogoUrl(e.target.value)} placeholder={t('logo_url_placeholder')} className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm" />
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {rivals.map((r) => {
          ...
          return (
            <Link key={r.id} to={`/save/${saveId}/rivals/${r.id}`} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-4 hover:border-emerald-400 hover:shadow-sm transition flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">{total} {t('matches')} · {r.wins}W {r.draws}D {r.losses}L · {r.goals_for}-{r.goals_against}</div>
              </div>
            </Link>
          );
        })}
        {rivals.length === 0 && (<div className="sm:col-span-2 text-slate-500 border border-dashed border-slate-300 dark:border-slate-700 rounded p-8 text-center bg-white dark:bg-slate-900">No rivals yet.</div>)}
      </div>
    </div>
  );
}

...
export default function RivalsPage() {
  ...
  async function load() {
    ...
    const [{ data: rs }, { data: ms }] = await Promise.all([
      supabase.from('rivals').select('*').eq('save_id', saveId).order('rival_team', { ascending: true }),
      supabase.from('matches').select('rival_id, result, home_goals, away_goals, home_team, away_team').eq('save_id', saveId),
    ]);
    const stats: RivalStats[] = (rs ?? []).map((r) => {
      ...
      for (const m of own) {
        ...
        // approximate GF/GA using rival_team vs. home_team
  ...
  return (
    <div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-4 mb-6 grid gap-2 sm:grid-cols-4">
        <input value={team} onChange={(e) => setTeam(e.target.value)} placeholder={t('rival_placeholder')} className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm text-slate-900 dark:text-slate-100 sm:col-span-2" />
          ...
          return (
            ...
            >
              <div className="w-14 h-14 rounded-lg flex items-center justify-center overflow-hidden shrink-0" style={{ background: r.rival_color ?? '#0f172a' }}>
                {logo ? <img src={logo} alt="" className="w-full h-full object-cover" /> : <span className="text-white font-bold">{r.rival_team.slice(0,2).toUpperCase()}</span>}
              </div>

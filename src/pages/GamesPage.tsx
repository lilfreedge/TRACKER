    ...
    <div>
      ...
      <div className="flex gap-2 mb-4 items-center">
        <div className="flex-1 relative">
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search games…" className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg pl-10 pr-3 py-2.5 text-sm" />
                ...
                <Link to={`/game/${encodeURIComponent(g.name)}`} className="block">
                  <div className={`aspect-[3/4] relative overflow-hidden bg-gradient-to-br ${gt.color}`}>
                    ...
                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent p-4">
                      ...
                      <div className="text-white/70 text-xs mt-0.5">{t('saves_count', saveCounts[g.name] ?? 0)}</div>
                    </div>
                  </div>
                </Link>

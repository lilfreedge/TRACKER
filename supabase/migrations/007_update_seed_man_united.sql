-- ============================================================
-- Complete data for the 19 Manchester United players (2037-2038)
-- Based on EAFC 26 public data (as of user's game state).
-- Idempotent: uses save name + player name + season label.
-- ============================================================
do $$
declare
  v_season_id uuid;
begin
  ...
  where cs.name = 'ARSEN-LEVER-DEPOR'
    and se.label = '2037-2038';

  if v_season_id is null then
    raise notice 'Season 2037-2038 not found — run 002 first.';
    return;
  end if;

  -- Enrich club_since_year on the season header
  update seasons set club_since_year = 2037 where id = v_season_id;

  -- Best-effort fill for the 19 players (jersey, age, nationality, since)
  ...
  raise notice 'Seed enriched for 2037-2038.';
end $$;

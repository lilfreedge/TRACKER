do $$
declare v_save_id uuid; v_season_id uuid;
begin
  select id into v_save_id from career_saves where owner_email='felipetorreira2@gmail.com' and name='ARSEN-LEVER-DEPOR';
  if v_save_id is null then insert into career_saves (owner_email,name,notes) values ('felipetorreira2@gmail.com','ARSEN-LEVER-DEPOR','Imported from Google Sheet') returning id into v_save_id; end if;
  select id into v_season_id from seasons where save_id=v_save_id and label='2037-2038';
  if v_season_id is null then insert into seasons (save_id,label,team_name_snapshot,team_color,team_text_color,formation,start_date,is_current) values (v_save_id,'2037-2038','MANCHESTER UNITED','#DA291C','#FBE122','4-3-3','2037-08',true) returning id into v_season_id; end if;
  delete from squad_players where season_id=v_season_id;
  insert into squad_players (season_id,name_snapshot,position,ovr,age,nationality_snapshot,role,formation_slot,jersey) values
  ...
  insert into squad_players (season_id,name_snapshot,position,ovr,role) values
